    ...
    execute(natlang) {
        return __awaiter(this, void 0, void 0, function* () {
            for (const ast of (0, Parser_1.getParser)(natlang).parseAll()) {
                ...
                const topLevelEntities = clause.
                    entities
                    .map(e => ({ e, c: clause.about(e) }))
                    .filter(x => !x.c.flatList().some(i => i.predicate === 'of'
                    && i.args[0] === x.e))
                    .map(x => x.e);
                ...
                //             && (i as BasicClause).args[1] === topLevelEntities[0]))
                //         .map(x => x.e)
                // console.log({ secondLevelEntities })
                const ownedBy = (owner) => clause
                    .entities
                    .map(e => ({ e, c: clause.about(e) }))
                    .filter(x => x.c.flatList().some(i => i.predicate === 'of'
                    && i.args[0] === x.e
