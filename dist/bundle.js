...
class EagerLexer {
    constructor(sourceCode, context) {
        ...
        this.context = context;
        this.tokens =
            this.joinMultiWordLexemes((0, Lexeme_1.stdspace)(sourceCode), context.config.lexemes)
                // .toLowerCase()
                .trim()
                .split(/\s+|\./)
                .map(s => !s ? '.' : s)
                .map(s => (0, Lexeme_1.respace)(s))
                .flatMap(s => (0, Lexeme_1.getLexemes)(s, context));
        this._pos = 0;
    }
    next() {
        this._pos++;
...
function getLexemes(word, context) {
    var _a;
    const lexeme = (_a = context.config.lexemes.filter(x => formsOf(x).includes(word)).at(0)) !== null && _a !== void 0 ? _a : { root: word, type: 'noun' };
    // ?? { root: word, type: 'any' }
