...
A popular criticism of OOP revolves around its reliance on Software Design Patterns to solve a specific set of problems. The natural way to solve a problem, it is argued, should be to solve it once and for all and then abstract away the solution for later re-use. Software Design Patterns on the other hand require rethinking through and reimplementing solutions for a very similar problem everytime it comes up again. It can also be shown that some of the original 23 GoF patterns for OOP languages can be entirely avoided when using languages with a different set of abstractions, for instance: higher order functions from Functional Programming. // graham, revenge of nerds
