...
The evolution of programming languages also mirrors that of natural languages; to some degree at least. It all begins with a population of original languages, then new ones arise; features are transferred from one language, or language family, to another. Eventually, some of these languages "die out", because people no longer "speak" them @chowdhary2007evolution. Some are even critical of this state of affairs @mandrioli2015programming, they think the design of new languages should take on a more principled approach: more focus on the theoretical principles, less of these organically (and haphazardly) growing languages, and perhaps the "forced retirement" of some of the worst offenders @douglasCrockfordJS.
...
The four programming paradigms that we mentioned by no means exhaust all of the known paradigms, let alone all of the possible yet undiscovered ones. It is nonetheless useful and instructive to take a closer look at the history and evolution of these few, alongside the programming languages that championed them and pioneered their use. This is what we shall attempt to do in the following pages. 
...
Despite these precursors in the mid 19th century, we will have to wait a little while more to see the birth of the man conventionally credited as the inventor of the first programming language: German computer scientist Konrad Zuse (1910-1995).
...
This is precisely why assembly languages were invented shortly after that. An assembly language is usually a thin abstraction on top of the underlying machine code, the latter still being the only language which the bare metal can run out of the box. But assembly languages are much easier for a human to understand than machine code, because they provide alphabetical aliases for the numerical opcodes, and because they manage memory addresses automatically; relieving the programmer from the burden of having to keep track of them in his/her head.
...
Back in the day, computers were mostly used to perform scientific computations, and floating point operations, (which just means operations on decimal numbers up to a certain level of precision), were thus one of the most important kinds of operations for most programs written at the time; and to this day we still sometimes measure computing power in FLOPS (Floating Point Operations per Second).
...
Fortran survives to this day in some scientific computation communities, with heavy modifications from the language that originally came out in 1956; the original version of Fortran developed by John Backus (1924-2007) and his team at IBM supported basic arithmetics, limited length variable names, subroutines, the do-loop, and a construct known as the "arithmetic if", which branched to different parts of the program based on the value of an arithmetical expression.

Fortran lacked many of the language facilities we take for granted today. There was no incremental compilation: all of the parts of a program had to be recompiled from scratch all the time, and, given the occasional unreliability and slowness of the 704 /* was it compiled on the same machine it ran on?? */, this meant restricted program sizes. It also didn't have any dynamic memory allocation capabilities, (which weren't essential for batch-processing scientific computations anyway).

Overall, Fortran is an example among many of a language born with a specific purpose (doing math efficiently on the IBM 704) that later evolved, incorporating new constructs, and getting ported over to numerous platforms. It is also a prime example of an imperative, procedural language.
...
The extensive use of GOTOs was famously criticised by Dijkstra in his 1968 letter titled: "Go To Statement Considered Harmful" @dijkstra1968letters. He argued that humans are better suited at understanding static processes: that they have a poor grasp of those processes that evolve in time.
...
Lisp (List Processor) was born with very different goals in mind than languages preceding it. It was designed by the American computer scientist and Artificial Intelligence researcher John McCarthy (1927-2011) in 1958. Incidentally, it is he who coined the term "Artificial Intelligence" (AI) back in those years. His wanted to design a language that made it easy to perform symbolic computations on lists. A list data-structure in computer science is a sequence of elements, which is not contiguous in memory; each element stores a value and a pointer to the location of the successor.

=== Advice Taker

McCarthy's aim at the time was to describe "Advice Taker", a hypothetical program he first introduced in his seminal paper from 1959: "Programs with Common Sense" @mccarthy1959programs. The program would be fed with data in the form of predicate calculus Well-formed formulas (WFFs) or "sentences", and it would be given a concrete goal to accomplish; it would've then had to reason from the premises it had available to arrive at a solution to the problem, drawing immediate conclusions from the premises and "taking action" in case the conclusions were found to be imperative sentences.
...
Unlike iteration, recursion doesn't require any reassignment of "counter" variables. Any iterative process (or "loop") must keep (at least) one counter which is incremented/decremented at every iterative step, similar to how a jury may count the loops the athletes run at a sports competition. 
...
If the language above is statically scoped, then when calling the function "bake" from whichever distant position in the code, "X" will always be "the oven", no matter what. But if the above language is dynamically scoped, well... it depends! If the function "bake" is called from a context where the name "X" has been redefined to mean something else (suppose it is now: "the freezer") then the cake will be baked by placing it in the freezer!

Dynamic versus static scoping thus behave differently in the presence of implicit arguments to a function; ie: when a function reads a variable from the outer scope. The reader can see why dynamic scoping is thought to be a little less intuitive than static scoping; in spite of this, it is still deemed useful by some programmers in certain situations @emacspaper.
...
The performance of purely functional languages has improved over the years, making them a viable alternative to their imperative counterparts in many cases. The debate over the purported "naturalness" of the imperative approach over the functional one, or viceversa, is still very much alive today.
...
On the contrary, pass by name doesn't. Pass by name is akin to a textual substitution, within the function's body, of every appearence of the parameter's name with the argument's text.

The implication of this, is: that formulas can be passed in as arguments. They are not evaluated before the function's body is executed, but rather during its execution. This makes it possible to exploit a technique known as Jensen's Device, from Danish computer scientis Jørn Jensen (1925-2007).

For instance, if the formula `w*x[i]` is passed in as a parameter to a function that performs a loop over `i` from `0` to `n` and sums up the values, the sum of the weighted values of an array can be computed for any arbitrary value of `w`, or, for that matter, any arbitrary expression that is passed into the same parameter. In modern languages, a lambda expression (anonymous function) is typically used to a similar effect.
...
In closing, we must remember that many mission critical systems to this day depend on large COBOL codebases; this is why, despite the kind of reputation it has earned during the last 60+ years of its existence, COBOL is still a hot topic today @cobolImportance.
...
We think the main lesson to take away from PL/I is that a language shouldn't incorporate too many features that can't be well integrated together. It is best to design with a minimal set of orthogonal features, that allow for future extension by the users.
...
Smalltalk originated in Alan Kay's late 1960s Ph.D. dissertation. Alan Kay, partly inspired by Simula, is credited for putting OOP on a firm theoretical basis. The idea of message passing is paramount to Kay's concept of object orientation: everything is an object, and all objects communicate with each other exlusively by passing messages. 

He was inspired by his knowledge of how cells worked in living organisms; and had envisioned, much before the personal computer really took off, a huge global network of intercommunicating computers. Analogously, the objects he described could be seen as a network of virtual computers.
...
This is something that has lead to problems, compounded by the possibility of creating deep and complicated inheritance hierarchies that are hard to maintain. This has generally turned inheritance into a discouraged construct, an idea which is usually manifested in the "composition over inheritance" motto @hunt2003inheritance.
...
Despite the fair criticism, from illustrous critics, OOP remains the dominant approach in the software industry. It has brought programming abstractions a little bit closer to the human mind, with its insistence on the familiar "object" metaphor, and on the integration of data with behavior.
...
The universal quantifier declares a global property of all individuals that satisfy a given description; for instance `∀X.(mortal(X) <- man(X))`, where `<-` is an implication arrow, may express the idea that every man is mortal, ie: "X is a man, implies X is mortal".
...
A Horn clause has a left and a right handisde that are separated by an implication. The implication goes from right to left. The left hand side may only contain one single predicate, or be completely empty. When the left handside is empty, there is no implication, and the right handside is treated as a fact; for instance the formula `cat(luna)` from before, which states the simple fact that "Luna is a cat".

When the left handside is _not_ empty, then the Horn clause is actually what we would think of as an implication; you could also roughly think of it as a "definition" of the meaning of the left handside in terms of the meaning of the right handside; for instance: `father(X,Y) :- parent(X,Y), male(X)` means that: "X is the father of Y, when X is the parent of Y and X is male"; every variable in a Horn clause is implicitly universally quantified.
...
For instance, querying for `father(X, luke)` will attempt to find Luke's father using the information contained in the source file; `father(walker, luke)` will check if Walker is Luke's father, returning `yes` in case this is supported by the information available, and `no` otherwise. This process of substituting variables and trying to match formulas is known as Unification, and its implementation can be rather complex.
