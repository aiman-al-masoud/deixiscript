...
It seems that Alan Kay considers this, and other aspects of the modern popular approach to OOP, as a significant departure from what he originally envisioned in Smalltalk. For example, in the way many subsequent languages conflated polymorphism (common interfaces) with inheritance (code reuse) through the mechanism of classes; something which has lead to problems, especially with large inheritance hierarchies, and has generally turned inheritance into a discouraged or even "dangerous" construct, an idea which is usually manifested in the "composition over inheritance" motto.
