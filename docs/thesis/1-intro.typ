...
Chomsky argues that while systems like GPT are very good at capturing statistical patterns in natural language, they also work just as well with any "impossible language", ie: with artificial grammars that infants don't learn spontaneously; making them an unreliable model of (at least) human language acquisition.

Moreover, we can add, such models need huge amounts of data to achieve a level of linguistic competence that is even comparable to that of a human being; a great human author doesn't need to read a _million_ books before writing his/her _magnum opus_, maybe a couple thousands or less.
