...
Lastly, if the constraints are even more vague than that, if they do not even tentatively try to provide necessary conditions for their solution, then they are "trivial" and the problems they describe are termed "improvisational"; examples of such problems are common in the domain of aesthetics (such as the arts).
...
Back in 1950, the British mathematician, logician and cryptanalist Alan Turing (1912-1954), set out on the ambitious journey of determining whether sentient behaviour was "provably computable" @turing2009computing, @smith2006history.
...
It was recently reported (2022-2023) that some Large Language Models (LLMs), based on Deep Learning techniques, such as Google's LaMDA @turingtestobsolete and OpenAI's GPT @gptbroketuring have succeeded at passing the Turing Test, an ambitious feat which had remained largely unaccomplished for decades prior to that @smith2006history. If this is all accurate, it means that we are now living in the post-Turing Test era.
...
Numerous objections and counter-objections to the argument have been raised since then, and there is still no general consensus on the conclusions and validity of the argument @sep-chinese-room. It is nonetheless a powerful and intuitive case for the idea that what computers are doing when they "think" is fundamentally different from what humans do (or at least, what the writer of this paragraph in his subjective experience does).
...
We think this is an interesting paradigm, because it could be extended to systems with various "agents" (similar to objects in OOP) @laffra1991constraints, and these independent actors could all be trying to fulfill their own goal (constraint) different from the goals of the others, by executing some basic actions declared procedurally by the programmer or perhaps learned through the concatenation of more basic actions, and abiding by the rules (or circumstances) declared by the programmer in natural language.
