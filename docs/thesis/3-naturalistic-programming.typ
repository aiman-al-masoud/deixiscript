...
So, if the "Penguin" class cannot fly, neither can the "Bird" superclass. The "Sparrow" class might. But is it "natural" to say that "birds (generally) do not fly"? As it is argued in other places @brachman2022machines: no, this is not the natural way humans organize concepts. A more naturalistic approach, would be to (somehow) declare a set of defeasible (revisable) defaults for the concepts at hand; in other words, any bird can fly, until proven that it cannot.
...
Prompt Injection (a snowclone of "SQL Injection") is a direct effect of the flexibility of the input medium (unconstrained natural language); any sentence could be treated as data or as an instruction, a property that is similar to what is known as "Homoiconicity" in some programming languages: “In a homoiconic language, the primary representation of programs is also a data structure in a primitive type of the language itself” @ceravola2021json.
...
The effectiveness of LLMs at performing this and related tasks was the object of study of an April 2023 paper @poldrack2023ai. 
...
We think perhaps this could also be helpful in gaining some better empirical understanding of what some researchers call "programmatic semantics" (the way natural language structures map to traditional programming language structures), which we also talked about in @metafor.
...
As we have seen, there exists a Precision/Expressivity tradeoff @kuhn2014survey. We have also already mentioned in the introduction that Dijkstra wrote a short piece in 1978 in which he was critical of what he called "natural language programming". 
...
As explained in the article "From: Language and Communication Problems in Formalization" @fantechi2021language, a natural language's sources of imprecision are multiple: syntax, anaphora (implicit references to previously mentioned entities), vagueness, comparatives and superlatives, disjunctions ("do this _or_ do that"), escape clauses ("if _possible_"), usage of a weak verb (may, may not), quantifiers (the scope of quantifiers can be unclear), underspecification and passive voice (passive voice may omit the doer of the action).
...
It is important however to note that nowadays computers are no longer just "computing machines", but also tools for document and multimedia consumption, gaming consoles, video-editing devices, office work stations, etc.
...
One of the criticisms we wish to add to this list relates to the observation that "many important precise documents are written in natural language" @nelson2006natural. We think that this claim needs a little clarification, because the kind of language employed in works such as legal documents and scientific literature is a special (slightly more formal) subset of the natural language we speak everyday.
...
As some argue, ideas from the programming paradigms we discussed, ultimately trace their origin to some idea in natural language @pulido2019model: they are part of this original "toolbox" (natural language's descriptive mechanisms); and in any case, we can successfully describe these programming paradigms to someone who has never heard of them before, using natural language.
...
(1) Gracefully integrate high-level concepts and low-level concepts @osmosianblog, offer a way to express mathematical ideas in mathematical notation, and relate those quantitative ideas to "higher-level" qualitative ones;

(2) Be readable to the average English speaker (if based on English) @nelson2006natural, @knoll2006pegasus, who should be able to correctly guess the meaning of a full (at least a reasonably small) program written in it; and not just an individual statement here and there, but the full document;

(3) Allow for the moderate use of implicit references @knoll2006pegasus, @pulido2019model ("the red cat", "the yellow dog", "the last number", etc.), but not to the point of introducing anaphoric ambiguities; after all, we tend to naturally disambiguate (we paraphrase sentences) to make ourselves clear to other human beings who did not "get us" the first time; it is fair to expect a formal register (it is a program specification after all) to require less of these implicit references, than casual talk between friends;
