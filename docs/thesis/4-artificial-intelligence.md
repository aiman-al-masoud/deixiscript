...
A point that the two approaches have in common, in a sense, is that they can be both regarded as declarative programming paradigms. As we've discussed in the section dedicated to programming languages, a computer can be told _how_ to do something (imperative), or it can be (just) told to do that thing (declarative). An extreme case of declarative programming is when the computer merely receives a description of the problem and has to devise a solution of its own. In this sense, both kinds of AIs are declarative: the idea is to avoid explicitly coding a behavior that may be beyond our practical reach with more traditional programming abstractions.
