import { LLangAst, AstMap, isLLangAst, astsEqual, WmAtom } from "./types.ts";
...
export function subst(formula: LLangAst, arg: unknown): LLangAst {
    ...
    if (arg instanceof DeepMap) {
        ...
        return subs.reduce((f, s) => substOnceNew(f, s[0], s[1]) as LLangAst, formula)
...
//             return {
//                 type: 'arbitrary-type',
//                 head: substOnce(ast.head, oldTerm, replacement),
//                 description: substOnce(ast.description, oldTerm, replacement),
//             }
...
function substOnceNew(
    ...
    if (ast instanceof Array) {
        return ast.map(e => substOnceNew(e, oldTerm, replacement) as LLangAst)
    }
    ...
    const newEntries = Object.entries(ast)
        ...
        .map(e => [e[0], substOnceNew(e[1] as LLangAst, oldTerm, replacement)] as const)
...
// const x = $('capra').isa('scemo').when(true).and(false).$
// console.log(substOnceNew(x, $('capra').$, $('goat').$))
